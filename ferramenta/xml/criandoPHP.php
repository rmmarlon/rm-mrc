<?
	...
	$php = '<? ' . "\n";
		...
		$php .= "\t  \n";
		$php .= "\t" . 'class ' . strCapitalize($nome) . ' extends DataRegistro{' . "\n";
			$php .= "\t \t" . '#Atributos' . "\n";
				...
				if($valor == 'cpf' || $valor == 'Cpf' || $valor == 'cnpj' || $valor == 'Cnpj' || $valor == 'Rg' || $valor == 'rg'){
					...
					if(substr($valor,0,4) == 'data'){
						...
						if(substr($valor,0,2) == 'id'){
							...
							$php .= "\t \t\t" . 'return $this->' . strtoupper(substr($valor,0,1)) .strtolower(substr($valor,1,1)). strtoupper(substr($valor,2,1)).strtolower(substr($valor,3)) . ';' . "\n";
							$php .= "\t \t" . '}' . "\n";
	...
	if(file_put_contents("output/" . strCapitalize($nome) . ".php", $php)){
		echo "Classe ".strCapitalize($nome)." criado com sucesso <br>";
		#copia o html da url requisitada echo file_get_contents("http://www.google.com.br");
	} else{ 
		echo "erro";
	}
	...
	$dao = '<?' . "\n";
		...
		$dao .= "\n";
		$dao .= "\t" . 'class ' . strCapitalize($nome) . 'DAO{' . "\n";
		...
		$dao .= "\t" . '}' . "\n";
	$dao .= '?>' . "\n";
	if(file_put_contents("output/" . strCapitalize($nome) ."DAO.php", $dao)){
		echo "Arquivo ".strCapitalize($nome)." criado com sucesso <br>";
	}
